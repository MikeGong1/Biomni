...
- 80% primary-limit headroom when GitHub reports an unauthenticated 60/hour
  limit;
...
credential helper. It never writes or prints the token. Queue state contains only
timestamps and GitHub rate-limit counters under
`/private/tmp/biomni-github-api-queue` by default.
...
REST responses persist `ETag`/`Last-Modified` sidecars and reuse cached bodies on
304. GraphQL responses use a query+variables SHA-256 cache for five minutes.
